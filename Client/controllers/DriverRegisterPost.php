...
$carDetails = FormUtils::getPostString($req->body('carDetails'));
$seats = FormUtils::getPostInt($req->body('seats'));
$payment = FormUtils::getPostString($req->body('payment'));
$available = FormUtils::getPostString($req->body('available'));
...
else
{
    ...
    if (!$carDetails['is_valid']) {
        $form_error_messages['carDetails'] = "Valid Car Details are required";
    }
    if (!$seats['is_valid']) {
        $form_error_messages['seats'] = "A Valid number of seats are required";
    }
    if (!$payment['is_valid']) {
        $form_error_messages['payment'] = "Valid payment required";
    }
    if (!$available['is_valid']) {
        $form_error_messages['available'] = "Availability required";
    }
...
if (!$form_was_posted || count($form_error_messages) > 0) 
{
    ...
    $res->render('main', 'driverRegister', [
        'pageTitle' => 'Register',
